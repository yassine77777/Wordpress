...
function gamipress_bbp_activity_recount_activities( $response, $loop, $limit, $offset ) {
    ...
    if( $loop === 0 && $posts_count > $limit ) {
        ...
        $response['message'] = sprintf( __( '%d items found, recounting...', 'gamipress-bbpress-integration' ), $posts_count );
        $response['log'] = sprintf( __( '%d items found, recounting...', 'gamipress-bbpress-integration' ), $posts_count ) . "<br>\n";
    ...
    if( $recounted_posts <= $posts_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining items to finish recount', 'gamipress-bbpress-integration' ), ( $posts_count - $recounted_posts ) );
    }
...
function gamipress_bbp_activity_recount_favorites( $response, $loop, $limit, $offset ) {
    ...
    if( $loop === 0 && $users_count > $limit ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d users found, recounting...', 'gamipress-bbpress-integration' ), $users_count );
        $response['log'] = sprintf( __( '%d users found, recounting...', 'gamipress-bbpress-integration' ), $users_count ) . "<br>\n";
    ...
    if( $recounted_users <= $users_count ) {
        $response['run_again'] = true;
        $response['message'] = sprintf( __( '%d remaining users to finish recount', 'gamipress-bbpress-integration' ), ( $users_count - $recounted_users ) );
    }
