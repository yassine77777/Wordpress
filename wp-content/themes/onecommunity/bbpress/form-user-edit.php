...
<form id="bbp-your-profile" action="<?php bbp_user_profile_edit_url( bbp_get_displayed_user_id() ); ?>" method="post" enctype="multipart/form-data">
	...
	<h2 class="entry-title"><?php esc_attr_e( 'Name', 'bbpress' ) ?></h2>

	<?php do_action( 'bbp_user_edit_before' ); ?>

	<fieldset class="bbp-form">
		<legend><?php esc_attr_e( 'Name', 'bbpress' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_name' ); ?>

		<div>
			<label for="first_name"><?php esc_attr_e( 'First Name', 'bbpress' ) ?></label>
		...
		<div>
			<label for="last_name"><?php esc_attr_e( 'Last Name', 'bbpress' ) ?></label>
		...
		<div>
			<label for="nickname"><?php esc_attr_e( 'Nickname', 'bbpress' ); ?></label>
		...
		<div>
			<label for="display_name"><?php esc_attr_e( 'Display Name', 'bbpress' ) ?></label>
	...
	<h2 class="entry-title"><?php esc_attr_e( 'Contact Info', 'bbpress' ) ?></h2>

	<fieldset class="bbp-form">
		<legend><?php esc_attr_e( 'Contact Info', 'bbpress' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_contact' ); ?>

		<div>
			<label for="url"><?php esc_attr_e( 'Website', 'bbpress' ) ?></label>
	...
	<h2 class="entry-title"><?php bbp_is_user_home_edit() ? esc_attr_e( 'About Yourself', 'bbpress' ) : esc_attr_e( 'About the user', 'bbpress' ); ?></h2>

	<fieldset class="bbp-form">
		<legend><?php bbp_is_user_home_edit() ? esc_attr_e( 'About Yourself', 'bbpress' ) : esc_attr_e( 'About the user', 'bbpress' ); ?></legend>

		<?php do_action( 'bbp_user_edit_before_about' ); ?>

		<div>
			<label for="description"><?php esc_attr_e( 'Biographical Info', 'bbpress' ); ?></label>
	...
	<h2 class="entry-title"><?php esc_attr_e( 'Account', 'bbpress' ) ?></h2>

	<fieldset class="bbp-form">
		<legend><?php esc_attr_e( 'Account', 'bbpress' ) ?></legend>
		...
		<div>
			<label for="email"><?php esc_attr_e( 'Email', 'bbpress' ); ?></label>
			...
			if ( !empty( $new_email ) && $new_email !== bbp_get_displayed_user_field( 'user_email', 'edit' ) ) : ?>
				...
				<span class="updated inline">
					...
					<?php printf( esc_attresc_attr__( 'There is a pending email address change to <code>%1$s</code>. <a href="%2$s">Cancel</a>', 'bbpress' ), $new_email['newemail'], esc_url( self_admin_url( 'user.php?dismiss=' . bbp_get_current_user_id()  . '_new_email' ) ) ); ?>
		...
		<div id="password">
			<label for="pass1"><?php esc_attr_e( 'New Password', 'bbpress' ); ?></label>
			<fieldset class="bbp-form password">
				...
				<span class="description"><?php esc_attr_e( 'If you would like to change the password type a new one. Otherwise leave this blank.', 'bbpress' ); ?></span>

				<input type="password" name="pass2" id="pass2" size="16" value="" autocomplete="off" tabindex="<?php bbp_tab_index(); ?>" />
				<span class="description"><?php esc_attr_e( 'Type your new password again.', 'bbpress' ); ?></span><br />

				<div id="pass-strength-result"></div>
				<span class="description indicator-hint"><?php esc_attr_e( 'Your password should be at least ten characters long. Use upper and lower case letters, numbers, and symbols to make it even stronger.', 'bbpress' ); ?></span>
			</fieldset>
		</div>
	...
	<?php if ( current_user_can( 'edit_users' ) && ! bbp_is_user_home_edit() ) : ?>
		...
		<h2 class="entry-title"><?php esc_attr_e( 'User Role', 'bbpress' ) ?></h2>

		<fieldset class="bbp-form">
			<legend><?php esc_attr_e( 'User Role', 'bbpress' ); ?></legend>
			...
			<?php if ( is_multisite() && is_super_admin() && current_user_can( 'manage_network_options' ) ) : ?>
				...
				<div>
					<label for="super_admin"><?php esc_attr_e( 'Network Role', 'bbpress' ); ?></label>
					<label>
						...
						<?php esc_attr_e( 'Grant this user super admin privileges for the Network.', 'bbpress' ); ?>
					</label>
				</div>
	...
	<fieldset class="submit">
		<legend><?php esc_attr_e( 'Save Changes', 'bbpress' ); ?></legend>
		<div>

			<?php bbp_edit_user_form_fields(); ?>

			<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_user_edit_submit" name="bbp_user_edit_submit" class="button submit user-submit"><?php bbp_is_user_home_edit() ? esc_attr_e( 'Update Profile', 'bbpress' ) : esc_attr_e( 'Update User', 'bbpress' ); ?></button>
		</div>
	</fieldset>
