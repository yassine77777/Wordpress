			...
			<fieldset class="bbp-form">
				<legend>
					...
					<?php
						if ( bbp_is_forum_edit() )
							printf( esc_attr__( 'Now Editing &ldquo;%s&rdquo;', 'bbpress' ), bbp_get_forum_title() );
						else
							bbp_is_single_forum() ? printf( esc_attr__( 'Create New Forum in &ldquo;%s&rdquo;', 'bbpress' ), bbp_get_forum_title() ) : esc_attr_e( 'Create New Forum', 'bbpress' );
					?>
				...
				<?php if ( !bbp_is_forum_edit() && bbp_is_forum_closed() ) : ?>
					...
					<div class="bbp-template-notice">
						<p><?php esc_attr_e( 'This forum is closed to new content, however your account still allows you to do so.', 'bbpress' ); ?></p>
					</div>
				...
				<div>
					...
					<p>
						<label for="bbp_forum_title"><?php printf( esc_attr__( 'Forum Name (Maximum Length: %d):', 'bbpress' ), bbp_get_title_max_length() ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_type"><?php esc_attr_e( 'Forum Type:', 'bbpress' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_status"><?php esc_attr_e( 'Status:', 'bbpress' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_visibility"><?php esc_attr_e( 'Visibility:', 'bbpress' ); ?></label><br />
					...
					<p>
						<label for="bbp_forum_parent_id"><?php esc_attr_e( 'Parent Forum:', 'bbpress' ); ?></label><br />

						<?php
							bbp_dropdown( array(
								'select_id' => 'bbp_forum_parent_id',
								'show_none' => esc_attr__( '(No Parent)', 'bbpress' ),
					...
					<div class="bbp-submit-wrapper">
						...
						<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_forum_submit" name="bbp_forum_submit" class="button submit"><?php esc_attr_e( 'Submit', 'bbpress' ); ?></button>
...
<?php elseif ( bbp_is_forum_closed() ) : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php printf( esc_attr__( 'The forum &#8216;%s&#8217; is closed to new content.', 'bbpress' ), bbp_get_forum_title() ); ?></p>
		</div>
	</div>
...
<?php else : ?>
	...
	<div id="no-forum-<?php bbp_forum_id(); ?>" class="bbp-no-forum">
		<div class="bbp-template-notice">
			<p><?php is_user_logged_in() ? esc_attr_e( 'You cannot create new forums.', 'bbpress' ) : esc_attr_e( 'You must be logged in to create new forums.', 'bbpress' ); ?></p>
		</div>
	</div>
